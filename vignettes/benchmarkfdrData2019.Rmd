...
The data are downloaded from _Bioconductor_'s 
[ExperimentHub](http://bioconductor.org/packages/ExperimentHub) 
web resource. 

```{r}
hub = ExperimentHub()
query(hub, "benchmarkfdrData2019")
```
Alternatively, datasets can be loaded individually. Here, we 
load two _SummarizedBenchmark_ objects to illustrate the analyses that
